...
fn load_image(path: &str) -> RawImage2d<u8> {
    use std::fs::File;
    use std::io::BufReader;

    let file = File::open(path).expect("Texture file expected");
...
fn main() {
    // Build Window with glium GL context
    ...
    // GLSL code for vertex and fragment shader
    ...
    loop {
        ...
        target.draw((&positions, &normals), &indices, &program, &uniforms,
                    &params).unwrap();
        target.finish().unwrap(); // Flush frame to window
