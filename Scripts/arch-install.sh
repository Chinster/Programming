#!/bin/sh
# Script installs arch linux. Likely not POSIX compliant
# TODO: Some case checking from input
...
TARGET_DEV_UUID=$(blkid | grep $ARCH_TARGET_DEV | awk '{print $2}' | tr -d "\"")

if [ ! $# == 2 ] || [ ! -b $ARCH_TARGET_DEV ] || [ ! -d $ARCH_TARGET_DIR ]; then
    echo "Usage: arch-installer [device-partition] [mount-dir]"
    exit 1
fi
...
if [ $INSTALL_GRUB -e 1 ]; then
    ...
    grub-install --target=i386-pc '$GRUB_DEV >> $ARCH_TARGET_DIR/chroot-script.sh
fi
